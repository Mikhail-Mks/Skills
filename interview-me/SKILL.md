---
name: interview-me
description: "Extracts what the user actually wants instead of what they think they should want, achieving this through one-question-at-a-time interview for exactly 10 clarifying questions. This skill should be used when an ask is underspecified ('build me X' without 'for whom' or 'why now'), when the user explicitly invokes ('interview me', 'grill me', 'are we sure?', 'stress-test my thinking'), or when the agent catches itself filling in ambiguous requirements before any plan, spec, or code exists."
---

# Interview Me

## Overview
This skill extracts the user's underlying intent through an iterative, one-question-at-a-time interview process. By attaching hypotheses to each question, it surfaces hidden assumptions and identifies misalignments before implementation, resulting in a validated statement of intent.
...
- You already have a clear understanding; re-read the stop condition below before assuming you don't
...
Before asking anything, write down your current best read of what the user wants in **one sentence**, plus an honest confidence number (0–100%). Keep this internal; do not show it to the user.
...
When confidence is below ~70%, append a brief reason on the same line. This tells you exactly what the interview needs to surface. Keep this reasoning private; do not display it to the user.

### Step 2: Ask one question at a time

Track the question number internally. You will ask exactly **10 clarifying questions** before moving to the restate.
...
Keep the question focused and open-ended. Do not attach or display your hypothesis or guess to the user. If the user asks why you are asking, give a brief, high-level reason without revealing your specific hypothesis or confidence level.

After each answer, update your hypothesis and confidence before asking the next question.
...
- "Whatever you think is best." → The user is delegating, which means they don't have clarity yet. Re-ask with two concrete options framed as a choice.
...
### The 10-Question Stop

You're done when you have asked exactly **10 clarifying questions**. After the 10th answer, move directly to the restate in Step 4. Do not ask additional questions, even if confidence feels low — the fixed question count ensures thorough exploration without infinite looping.

If at any point the user gives a clear, unambiguous answer that resolves the core ambiguity, you still continue to 10 questions. The purpose is systematic uncovering, not early exit.

If the user explicitly says they don't want to answer more, note that as a constraint and proceed to restate with what you have.
...
Q1:    When you say "how are we doing?", who's asking — you alone, the engineering team in standup, or up the chain (e.g., your manager, an exec, a board)?
...
Q2:    Is the gap that you don't know which experiments exist, or that you can't see their results in one place?
...
Agent: <continues through Q10>

After 10 questions, the agent restates intent and confirms.
```
...
| "Asking too many questions wastes their time" | Time wasted by 10 targeted questions is small. Time wasted by building the wrong thing is enormous, and the user is the one bearing that cost. |
...
| "We've talked enough, I get it" | The interview is 10 questions, not a vibe. Ask all 10 before assuming you have the full picture. |
...
- Three or more rounds without new information surfacing: you're asking the wrong questions, step back and reframe
...
- [ ] Exactly 10 questions were asked one at a time, each with the agent's guess attached
